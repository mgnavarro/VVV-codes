...
pdfEN1 = PdfPages('Volumetric_density_profile_all.pdf')
...
for file in os.listdir('/Volumes/Gabriela_Pink/Data_FIRST_MEGANMW/exec'):
    if file.startswith('1001369') and file.endswith('.dat'):
            ...
            cmz=(np.rad2deg(np.arctan((np.mean(z)*20)/8178)))
            ...
            distance=(np.rad2deg(np.arctan((z*20)/8178)))
            r= (( (long-cmx)**2) + ((lat-cmy)**2)  + ((distance-cmz)**2)     )**(0.5)
            ...
            for h in range(areas):
                ...
                rmax_=rdef[h]+(0.01/2)
                r_plot[h]=rdef[h]
                area_shell= ((4/3 * np.pi * (rmax_**3)) - (4/3 * np.pi * (rmin_**3)))
            ...
            plt.ylabel(r'$\rho_{RRab} \: $ (per $deg^3$)', fontsize=14)
            plt.gca().set_yscale("log")
            plt.gca().set_xscale("log")
            txt="Caption: Density profile of all the 41 globular clusters in the last snapshot of the simulation. The black distribution correspond to the density profile for all the \n particles that belong to Globular Clusters. The blue, green and red represent the most negative energies, mid energies and energies in the peak of the distribution,\n  respectively. The convertion from parsecs to degrees was implemented assuming the Galactocentric distance R0 = 8.178 ± 13stat. ± 22sys. pc.\n  (The GRAVITY Collaboration, A&A 625, L10 (2019))."
            ...
            ax1.set_ylabel(r'$\rho_{RRab} \: $ (per $deg^3$)', fontsize=14)
            ...
            plt.text(0.5, 0.2, txt, wrap=True, horizontalalignment='center', fontsize=12)
